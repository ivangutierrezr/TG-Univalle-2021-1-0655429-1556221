...
# NOTA: REVISAR LINEAS 152 Y 401 PARA CAMBIAR LAS FUNCIONES DE FITNESS
...
def getFitnessByAckley(solution):
    x1 = solution[0]
    x2 = solution[1]
...
def getFitnessByRastrigin(solution):
    x1 = solution[0]
    x2 = solution[1]
    fitness = (x1**2) + (x2**2)
    return fitness
...
#función para retornar la media de fitness de los individuos de la población elegida
def getBestFitness():
    dataUniverses = universe1 + universe2 + universe3 + universe4
    
    dataUniverses.sort(key = lambda p: p["fitness"])
    elitePoints = dataUniverses[:chosen_size]

    newElitePoints = []

    for i in range(len(elitePoints)):
        newElitePoints.append(elitePoints[i]["coords"])

    newBoundsArr = np.array(newElitePoints)
...
#Función que se encarga de reproducir el universo. En este caso reproduce los individuos de mejor fitness organizados en orden ascendente de un universo con los de mejor fitness de otro universo organizados en orden ascendente
def reproduceUniverses(u1, u2):
    childs = []
    for i in range(chosen_size):
        # Se cruzan los individuos de los dos universos de acuerdo a su fitness. Ordenados vs ordenados
        # Cada nuevo hijo se generará a partir de la media entre los padres
        newX = (u1[i]["coords"][0] + u2[i]["coords"][0]) / 2
        newY = (u1[i]["coords"][1] + u2[i]["coords"][1]) / 2

        # Descomentar el fitness que se quiere aplicar
        fitness = getFitnessBySphere([newX, newY])
        # fitness = getFitnessByAckley([newX, newY])
        # fitness = getFitnessByRastrigin([newX, newY])
        
        newChild = {
            "fitness": fitness,
            "coords": [newX, newY],
            "parents": [u1[i]["keyP"], u2[i]["keyP"]]
        }
...
def runCode():
    ...
    for i in range(pop):
        ...
        #Descomentar el método que se quiere aplicar
        fitness = getFitnessBySphere([x[i], y[i]])
        # fitness = getFitnessByAckley([x[i], y[i]])
        # fitness = getFitnessByRastrigin([x[i], y[i]])
        point = {
            "keyP": i+1,
            "fitness": fitness,
            "coords": [x[i], y[i]],
...
plt.ylabel('Density')
plt.xlabel('Edges')
plt.title('Degree distribution Elite 21')
figHist = plt.gcf()

figHist.savefig('DistribucionGradoElite21.png')

plt.clf()

template = """Información de la red estrategia Elite 21
...
nx.write_gexf(G, "infoCEMElite21.gexf", version="1.2draft")
...
with open('infoCEMElite21Fitnesses.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(infoFitnesses)

# Creación de informe completo de Tiempos
with open('infoCEMElite21Times.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()
    writer.writerows(infoTimes)

# Creación de informe completo de Resumen
with open('ResumenElite21.csv', 'w', encoding='UTF8', newline='') as f:
...
ax.set_title("Estrategia Elite 21")
...
# plt.show()
plt.savefig("GraficoElite21.png", dpi=1600)